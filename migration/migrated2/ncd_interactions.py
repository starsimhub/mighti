...
class depression_obesity(ss.Connector):
    ...
    def step(self):
        ...
        for person in sim.people:
            if sim.diseases.depression.affected[person.uid]:
                # Get the relative risk from the CSV based on the person's age and sex
                person_sex = 'Female' if person.female else 'Male'
                relative_risk = ncd_interactions.get_interaction('Depression', 'Obesity', person.age, person_sex)
...
class hypertension_heart(ss.Connector):
    ...
    def step(self):
        ...
        for person in sim.people:
            if sim.diseases.hypertension.affected[person.uid]:
                # Get the relative risk from the CSV based on the person's age and sex
                person_sex = 'Female' if person.female else 'Male'
                relative_risk = ncd_interactions.get_interaction('Hypertension', 'HeartDisease', person.age, person_sex)
