...
class AdherenceEngine(ss.Module):
    ...
    def step(self):
        ...
        # Create adherence state if it doesn't exist (lazy initialization)
        if "adherence" not in st:
            # Create as a simple numpy array that we'll write to states
            adherence_arr = np.ones(n, dtype=float)
            st["adherence"] = adherence_arr
        ...
        adherence_clipped = np.clip(adherence, 0.0, 1.0)
        # Handle both FloatArr and numpy array cases
        if isinstance(st["adherence"], ss.FloatArr):
            st["adherence"][:] = adherence_clipped
        else:
            # If it's a numpy array, update in place
            st["adherence"] = adherence_clipped
