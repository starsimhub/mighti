...
class PrevalenceAnalyzer(ss.Analyzer):
    ...
    def init_pre(self, sim):
        ...
        super().init_pre(sim)
        npts = len(sim.timevec)  # Number of time points in the simulation
    ...
    def step(self):
        ...
        is_male = sim.people.male.raw
        alive = sim.people.alive.raw
        hiv_status = sim.people.hiv_status.raw
    
        # Store single-age population distribution at each time step
        age_distribution = np.histogram(ages[alive], bins=np.arange(0, 102, 1))  # Single-year resolution from 0 to 101
        ...
        for disease in self.diseases:
            ...
            for sex, sex_label in zip([0, 1], ['male', 'female']):
                for hiv, hiv_label in zip([True, False], ['with_HIV', 'without_HIV']):
