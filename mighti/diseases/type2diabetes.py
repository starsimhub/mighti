...
class Type2Diabetes(ss.NCD):
    def __init__(self, csv_path, pars=None, **kwargs):
        ...
        self.define_pars(
            ...
            init_prev=None,  # Accept external init_prev via update_pars
    ...
    def step(self):
        ...
        if self.ti % 5 == 0:  # log every 5 years
            print(f"[T2D step] Year {self.ti:.0f}: Affected={len(affected_uids)}, Deaths={len(deaths)}, Base p_death={base_p:.4f}, Mean rel_death={np.mean(rel_death):.4f}")
