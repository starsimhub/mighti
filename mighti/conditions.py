...
def initialize_conditions(df, chronic_list, acute_list, remitting_list, sis_list):
    ...
    global df_params, chronic, acute, remitting, communicable_diseases
    df_params = df  # Store the entire DataFrame globally
    chronic = chronic_list
    acute = acute_list
    remitting = remitting_list
...
def register_disease_classes():
    ...
    # Create Chronic Disease classes dynamically
    for disease in chronic:
        class_definition = type(
            disease,  # Class name (e.g., "Type2Diabetes")
            (ChronicDisease,),  # Base class (ChronicDisease)
            {
                "__init__": (lambda disease_name: 
                    lambda self, pars=None, **kwargs: 
                    ChronicDisease.__init__(self, disease_name, pars, **kwargs)
    ...
    # Create Acute Disease classes dynamically
    for disease in acute:
        class_definition = type(
            disease,  # Class name (e.g., "Flu")
            (AcuteDisease,),  # Base class (AcuteDisease)
            {
                "__init__": (lambda disease_name: 
                    lambda self, pars=None, **kwargs: 
                    AcuteDisease.__init__(self, disease_name, pars, **kwargs)
                )(disease)  # Correct lambda closure
            }
        )
        globals()[disease] = class_definition  # Add to global namespace
    ...
    # Create Remitting Disease classes dynamically
    for disease in remitting:
        class_definition = type(
            disease,  # Class name (e.g., "Type2Diabetes")
            (RemittingDisease,),  # Base class (RemittingDisease)
            {
                "__init__": (lambda disease_name: 
                    lambda self, pars=None, **kwargs: 
                    RemittingDisease.__init__(self, disease_name, pars, **kwargs)
                )(disease)  # Correct lambda closure
            }
        )
        globals()[disease] = class_definition  # Add to global namespace
...
def get_disease_parameters(disease_name):
    ...
    if df_params is None:
        raise ValueError("`df_params` is not initialized! Call `initialize_conditions(df, ncds, communicable_diseases)` first.")
...
class ChronicDisease(ss.NCD):
    ...
    def __init__(self, disease_name='ChronicDisease', csv_path=None, pars=None, **kwargs):
        super().__init__()
...
# Define Remitting Disease class
class RemittingDisease(ss.NCD):
    """ Base class for all remitting diseases. """

    def __init__(self, disease_name='RemittingDisease', csv_path=None, pars=None, **kwargs):
        super(RemittingDisease, self).__init__()
        self.disease_name = disease_name
        disease_params = get_disease_parameters(disease_name)
        ...
        self.define_states(
            ss.State('susceptible', default=True),
            ss.State('affected'),
            ss.State('reversed'),  # New state for remission
            ss.FloatArr('ti_affected'),
...
class AcuteDisease(ss.NCD):
    ...
    def __init__(self, disease_name='AcuteDisease', csv_path=None, pars=None, **kwargs):
        super(AcuteDisease, self).__init__()
        self.disease_name = disease_name
        disease_params = get_disease_parameters(disease_name)
