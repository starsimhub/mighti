...
def data_extraction(country, inityear, endyear):
    ...
    def extract_life_expectancy_data(life_expectancy_csv, output_csv, selected_year):
        # Read life expectancy data
        life_expectancy_data = pd.read_csv(life_expectancy_csv)
        
        # Extract data for the selected year
        selected_data = life_expectancy_data[['age', 'sex', str(selected_year)]]
        
        # Save the extracted data to CSV
        selected_data.to_csv(output_csv, index=False)
        print(f"Extracted life expectancy data for {selected_year} saved to {output_csv}")
    
    if __name__ == "__main__":
        # File paths
        male_csv = 'population_single_age_male.csv'
        female_csv = 'population_single_age_female.csv'
        output_csv = '{country}_age_distribution.csv'
        process_population_data(male_csv, female_csv, output_csv, country = country)
        
        male_csv = 'death_single_age_male.csv'
        female_csv = 'death_single_age_female.csv'
        output_csv = '{country}_deaths.csv'
        process_mortality_data(male_csv, female_csv, output_csv, country = country)
        
        # Load the age distribution data
        age_distribution_csv = '{country}_age_distribution.csv'
        
        # Load the mortality data
        mortality_deaths_csv = '{country}_deaths.csv'
        
        output_csv = '{country}_mortality_rates.csv'
        ...
        output_LE_csv = '{country}_life_expectancy_by_age.csv'
        process_life_expectancy_data(male_LE_csv, female_LE_csv, output_LE_csv, country)
    
        # Extract life expectancy data for a specific year
        life_expectancy_csv = '{country}_life_expectancy_by_age.csv'
        output_extracted_csv = 'extracted_life_expectancy_data.csv'
        selected_year = endyear
        extract_life_expectancy_data(life_expectancy_csv, output_extracted_csv, selected_year)