...
results_dir = f'results/calibration_{region}_'
...
# conditions = ['Type1Diabetes','Type2Diabetes','CardiovascularDiseases', 'ChronicKidneyDisease',
#               'CervicalCancer', 'ColorectalCancer', 'BreastCancer', 'LungCancer', 'ProstateCancer',
#               'AlcoholUseDisorder', 'InterpersonalViolence', 'RoadInjuries', 'ChronicLiverDisease',
#               'Asthma', 'COPD','MajorDepressiveDisorder']
...
def make_sim(disease_name, DiseaseClass):
    ...
    fertility = {'fertility_rate': pd.read_csv(f"../data/{region}_asfr.csv")}
    pregnancy = ss.Pregnancy(pars=fertility)
    death = ss.Deaths({'death_rate': pd.read_csv(f"../data/{region}_mortality_rates.csv"), 'rate_units': 1})
