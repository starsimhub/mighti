...
def make_sim():
    ...
    fertility_rate = {'fertility_rate': pd.read_csv('../data/nyc_asfr.csv')}
    pregnancy = ss.Pregnancy(pars=fertility_rate)
    death_rates = {'death_rate': pd.read_csv('../data/nyc_deaths.csv'), 'rate_units': 1}
    death = ss.Deaths(death_rates)  # Assuming death_rate is a yearly rate

    sexual = sti.StructuredSexual()
    maternal = ss.MaternalNet()

    prevalence_analyzer = mi.PrevalenceAnalyzer(prevalence_data=pd.read_csv('../data/nyc_prevalence.csv'), diseases=['HIV'])
...
def run_calib(calib_pars=None, total_trials=10, keep_db=False):
    ...
    data = pd.read_csv('../data/nyc_prevalence.csv')
